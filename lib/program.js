...
class Program {
  ...
  prepareOptions(options) {
    return load(_.omit(options, 'argv'), this[$spec] || [], this[$relaxed], this[$validations],
      err => {
        let output = process.env.NODE_ENV === 'development' ? (err.stack || err) : err;
        stderr('' + output);
        stderr('');
        this.usage(this[$spec]);
      });
  }
  ...
  usage(err) {
    ...
    let spec = this[$spec];
    try {
      stdout(`${ pkg.name } v${ pkg.version } - ${ pkg.description }`);
      stdout('');
      stdout(`Usage: ${pkg.name } ${ buildOptionList(spec) }`);
    } catch (err) {
      debug(err.stack);
    }
  }
